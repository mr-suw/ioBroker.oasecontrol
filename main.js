...
class Oasecontrol extends utils.Adapter {
    constructor(options = {}) {
        ...
        // @ts-expect-error: bind onStateChange to this instance
    ...
    async updateFmMasterStates(states) {
        ...
        this.log.debug(
            `outlet states polled (${states.s1}, ${states.s2}, ${states.s3}, ${states.s4}, ${states.s4_dim})`,
        );
    }
    ...
    async initFmMasterEgcStates() {
        ...
        if (roValOutlet1 == null) {
            this.setState(`${this.name}.${this.instance}.outlet1_readOnly`, false);
        }
        if (roValOutlet2 == null) {
            this.setState(`${this.name}.${this.instance}.outlet2_readOnly`, false);
        }
        if (roValOutlet3 == null) {
            this.setState(`${this.name}.${this.instance}.outlet3_readOnly`, false);
        }
        if (roValOutlet4 == null) {
            this.setState(`${this.name}.${this.instance}.outlet4_readOnly`, false);
        }

        //this.log.debug("creating FM master switch names...");
        if ((await this.getStateAsync('outlet1_name')) == null) {
            await this.createObj('outlet1_name', 'outlet1_name', 'state', 'text', 'string', true, true);
        }
        if ((await this.getStateAsync('outlet2_name')) == null) {
            await this.createObj('outlet2_name', 'outlet2_name', 'state', 'text', 'string', true, true);
        }
        if ((await this.getStateAsync('outlet3_name')) == null) {
            await this.createObj('outlet3_name', 'outlet3_name', 'state', 'text', 'string', true, true);
        }
        if ((await this.getStateAsync('outlet4_name')) == null) {
            await this.createObj('outlet4_name', 'outlet4_name', 'state', 'text', 'string', true, true);
        }

        //this.log.debug("initialize switch default names...");
        if ((await this.getStateAsync(`${this.name}.${this.instance}.outlet1_name`)) == null) {
            this.setState(`${this.name}.${this.instance}.outlet1_name`, 'outlet1');
        }
        if ((await this.getStateAsync(`${this.name}.${this.instance}.outlet2_name`)) == null) {
            this.setState(`${this.name}.${this.instance}.outlet2_name`, 'outlet2');
        }
        if ((await this.getStateAsync(`${this.name}.${this.instance}.outlet3_name`)) == null) {
            this.setState(`${this.name}.${this.instance}.outlet3_name`, 'outlet3');
        }
        if ((await this.getStateAsync(`${this.name}.${this.instance}.outlet4_name`)) == null) {
            this.setState(`${this.name}.${this.instance}.outlet4_name`, 'outlet4');
        }
    ...
    async onStateChange(id, state) {
        if (state && state.ack == false) {
            ...
            switch (idName) {
                case 'outlet1':
                    this.cmdReq.itemId = 0x00;
                    break;
                case 'outlet2':
                    this.cmdReq.itemId = 0x01;
                    break;
                case 'outlet3':
                    this.cmdReq.itemId = 0x02;
                    break;
                case 'outlet4':
                    this.cmdReq.itemId = 0x03;
                    break;
                case 'outlet4_dimmer':
                    this.cmdReq.itemId = 0x04;
                    break;
                default:
                    this.cmdReq.itemId = 0xff;
            }
            switch (state.val) {
                case false:
                    this.cmdReq.value = 0x00;
                    break;
                case true:
                    this.cmdReq.value = 0xff;
                    break;
                default:
                    this.cmdReq.value = Number(state.val);
            }
            if (this.cmdReq.itemId != 0xff && this.cmdReq.value <= 0xff && this.cmdReq.value >= 0x00) {
                //process cmd
                const sceneData = this.getOaseClient().createFmMasterSocketSceneSet(
                    this.cmdReq.itemId,
                    this.cmdReq.value,
                );
