...
class OaseClient {
    ...
    /**
     * Sends a discovery request and returns the parsed response.
     *
     * @param transport
     */
    async discoveryReq(transport = TransportType.TLS) {
        const response = await this.server.sendRequest(OasePacketTypes.DISCOVERY, Buffer.alloc(0), transport);
    ...
    /**
     * Sends an alive request and returns the parsed response.
     *
     * @param transport
     */
    async aliveReq(transport = TransportType.TLS) {
        const response = await this.server.sendRequest(OasePacketTypes.ALIVE, Buffer.alloc(0), transport);
    ...
    /**
     * Sends a TCP connection request for the specified port and returns the parsed response.
     *
     * @param port
     * @param transport
     */
    async tcpConReq(port, transport = TransportType.UDP) {
        ...
        const timestampArray = new Uint8Array(
            ...
            timestampBuffer.byteLength,
        );
        buffer.set(timestampArray, 3); // Copy 4-byte timestamp

        const response = await this.server.sendRequest(OasePacketTypes.TCP_REQ, buffer, transport);
    ...
    /**
     * Sends a password check request with the given password and returns whether the check was successful.
     *
     * @param pw
     * @param isPwUnicodeEncoded
     * @param transport
     */
    async checkDevicePwReq(pw, isPwUnicodeEncoded, transport = TransportType.TLS) {
        ...
        const response = await this.server.sendRequest(OasePacketTypes.PASSWORD_CHECK, pwBuffer, transport);
    ...
    /**
     * Sends a get live scene request with the given scene data and returns the parsed response.
     *
     * @param sceneData
     * @param transport
     */
    ...
    /**
     * Sends a set live scene request with the given scene data and returns whether the operation was successful.
     *
     * @param sceneData
     * @param transport
     */
    ...
    /**
     * Converts the input string to a 64-byte Uint8Array, padding with zeros if necessary. If isUnicodeEncoded is false, it decodes unicode escape sequences before conversion.
     *
     * @param input
     * @param isUnicodeEncoded
     */
    ...
    /**
     * Gets the current Unix timestamp as a 4-byte buffer in little-endian format.
     */
    getUnixTimestamp() {
    ...
    /**
     * Creates a buffer for the "Get Master Socket Scene" request with the appropriate scene ID and returns it.
     */
    ...
    /**
     * Creates a buffer for the "Set Master Socket Scene" request with the appropriate scene ID, scene type, scene length, and socket data, and returns it.
     *
     * @param socketIdx
     * @param socketVal
     */
    ...
    /**
     * Closes the UDP client connection if it exists.
     */
    close() {
        if (this.udpClient) {
    ...
    /**
     * Returns the UDP client instance.
     */
    getUdpClient() {
        return this.udpClient;
    }
